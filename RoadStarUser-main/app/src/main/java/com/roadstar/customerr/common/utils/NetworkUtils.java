...
public final class NetworkUtils {
     ...
     * HTTP timeout value
     */
    public static final int HTTP_TIMEOUT = 30000; // milli seconds
    /**
     ...
     * 4.  Read and return response from {@link HttpURLConnection} via {@link #getHttpURLConnectionResponse(HttpURLConnection, HttpRequestItem)}
     *
    ...
    public static String executeNetworkRequest(final HttpRequestItem params) {
        ...
        serviceName = AppUtils.ifNotNullEmpty(serviceName) ? "\"" + serviceName.substring(serviceName.lastIndexOf("/") + 1) + "\"" : "";
    ...
    private static String processHttpRequest(HttpRequestItem params, int retryCount) {
        ...
        try {
            ...
            response = getHttpURLConnectionResponse(connection, params);
     ...
     */
    private static String getHttpURLConnectionResponse(HttpURLConnection connection, HttpRequestItem params) throws IOException {
        String response;
        ...
        try {
            ...
            if (connection.getHeaderFields().containsKey("set-cookie") &&
                    ...
                    !params.getHttpRequestEndPoint().equalsIgnoreCase(AppConstants.APP_VERSION)) {
    ...
    private static HttpURLConnection initHttpURLConnection(HttpRequestItem params) throws Exception {
        ...
        Logger.info("Cookie_url", "" + params.getHttpRequestUrl());
...
//                httpURLConnection.setRequestProperty("Content-Length", Integer.toString(queryParams.length()));
                ...
                if (map != null && map.size() > 0) {
                    ...
                    dataOutputStream.write(jsonObject.toString().getBytes(CHARSET));
                }
...
//                Logger.warn(TAG, "No request params for HTTP POST");
    ...
    private static void addMultipartEntity(HttpRequestItem item, HttpURLConnection connection) throws Exception {
      ...
      //  File file = new File(item.getParams().get("file").toString());

        File file = new File(item.getParams().get(KEY_PICTURE).toString());
    ...
    //    outputStream.writeBytes("Content-Disposition: form-data; name=\"" + "image" + "\"; filename=\"" + file.getAbsolutePath() + "\"" + lineEnd);
        outputStream.writeBytes("Content-Disposition: form-data; name=\"" + "file" + "\"; filename=\"" + file.getAbsolutePath() + "\"" + lineEnd);
